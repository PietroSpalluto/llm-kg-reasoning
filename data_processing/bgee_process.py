...
df = pd.read_csv("../data/PrimeKG/bgee/Homo_sapiens_expr_advanced.tsv", sep='\t')

df = df[df.get('Anatomical entity ID').str.startswith('UBERON')]

df = df.get(['Gene ID', 'Gene name', 'Anatomical entity ID', 'Anatomical entity name',
             'Expression', 'Call quality', 'Expression rank'])
...
df.to_csv('../data/PrimeKG_processed/bgee/anatomy_gene.csv', index=False)
