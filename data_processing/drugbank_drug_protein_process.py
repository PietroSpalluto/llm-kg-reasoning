...
df_carrier = pd.read_csv('../data/PrimeKG/drugbank/drugbank_all_carrier_polypeptide_ids.csv/all.csv').query(
    'Species=="Humans"')
df_enzyme = pd.read_csv('../data/PrimeKG/drugbank/drugbank_all_enzyme_polypeptide_ids.csv/all.csv').query(
    'Species=="Humans"')
df_target = pd.read_csv('../data/PrimeKG/drugbank/drugbank_all_target_polypeptide_ids.csv/all.csv').query(
    'Species=="Humans"')
df_transporter = pd.read_csv('../data/PrimeKG/drugbank/drugbank_all_transporter_polypeptide_ids.csv/all.csv').query(
    'Species=="Humans"')

# WHAT'S VOCAB???? (find drugbank_vocabulary)
gene_vocab = pd.read_csv('../data/vocab/gene_map.csv', delimiter='\t')
db_vocab = pd.read_csv('../data/vocab/drugbank_vocabulary.csv')

up2ncbi = gene_vocab.get(['NCBI Gene ID(supplied by NCBI)', 'UniProt ID(supplied by UniProt)']).dropna().set_index(
    'UniProt ID(supplied by UniProt)').to_dict()['NCBI Gene ID(supplied by NCBI)']
ncbi2up = gene_vocab.get(['NCBI Gene ID(supplied by NCBI)', 'UniProt ID(supplied by UniProt)']).dropna().set_index(
    'NCBI Gene ID(supplied by NCBI)').to_dict()['UniProt ID(supplied by UniProt)']
...
df_prot_drug.to_csv('../data/PrimeKG_processed/drugbank/drug_protein.csv', index=False)
